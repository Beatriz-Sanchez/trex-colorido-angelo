...
function setup(){
  ...
  //cria solo
  solo = createSprite(windowWidth/2,windowHeight-10,1200,20);
  //adiciona imagem de solo
  solo.addImage("solo", imagemDoSolo);
  solo.scale = 1.1;
  
  //cria solo invisível
  soloInvisivel = createSprite(300,windowHeight-50,600,40);
  ...
  fimDoJogo = createSprite(windowWidth/2,windowHeight/2 -30,400,20);
  ...
  sol = createSprite(windowWidth-70,50,10,10);
...
function draw(){
  ...
  if(estadoJogo === JOGAR){
    ...
    if(keyDown('space') && trex.y>windowHeight-180 || touches.length > 0 && trex.y>windowHeight-180){
      trex.velocityY = -23; 
      somSalto.play();
      touches = [];
    }
...
function gerarNuvens(){
  ...
  if(frameCount %100 === 0){
    nuvem = createSprite(windowWidth + 30,100,40,10);
    ...
    nuvem.lifetime = windowWidth/nuvem.velocityX; + 30;
...
function gerarObstaculos(){
  ...
  if(frameCount %60 === 0){
    obstaculo = createSprite(windowWidth + 30,windowHeight - 140,10,40);
